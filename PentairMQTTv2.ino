...
WiFiClient cMQTT;
IPAddress ipMQTT(192, 168, 1, 254);
...
void loop() {
  ...
  WiFiClient cstatus = tstatus.available();
  WiFiClient cdebug = tdebug.available();
  WiFiClient cstream = tstream.available();
  cStatus = cstatus;
...
void processPentair(uint8_t* buffer, int len, WiFiClient client) {
...
void processIntellichlor(uint8_t* buffer, int len, WiFiClient client) {
  if (len == 8) {
...
void printByteData(uint8_t Byte, WiFiClient client) {
  client.print((uint8_t)Byte >> 4, HEX);
  client.print((uint8_t)Byte & 0x0f, HEX);
}

boolean mqttConnect() {
  if (mqtt.connect("PentairCntrl", mqttUser, mqttPassword)) {
...
bool mqttUpdate(WiFiClient client) {
  // See what changed!
...
bool mqttPubInt(char* Topic, int intMessage, bool retained, WiFiClient client) {
  char chrMessage[17];
